"""
Cloudflare D1 persistence layer for the AI Historical Character Prompt
Engineering Competition — participant workspace.

Handles: participant registration/timer state, per-round prompt + word count
storage, conversation history, and final submission locking.

--------------------------------------------------------------------------
Why D1 instead of local SQLite
--------------------------------------------------------------------------
Streamlit Community Cloud (and most hosted Streamlit environments) don't
guarantee a persistent local filesystem — the app's container can be
restarted or redeployed at any time, which would silently wipe a local
SQLite file (competition.db) mid-event. Cloudflare D1 is a real hosted SQL
database, so participant data survives restarts, redeploys, and multiple
app instances.

D1 has no native Python driver, so every function below talks to it over
Cloudflare's HTTP REST API (one query per call) rather than a local
sqlite3 connection. Every function keeps the exact same name, signature,
and return type as the old SQLite version, so app.py and gemini_ai.py
don't need any changes.

--------------------------------------------------------------------------
SETUP — you need a D1 database + API credentials
--------------------------------------------------------------------------
1. Cloudflare dashboard → Workers & Pages → D1 SQL Database → create (or
   reuse) a database. Note its Database ID (Settings tab).
2. Note your Account ID (top-right of the main dashboard).
3. My Profile → API Tokens → Create Token → give it D1 Edit permission.
4. Provide all three one of two ways:

   a) Local development — a .env file next to this one containing:

          CLOUDFLARE_ACCOUNT_ID=your-account-id
          CLOUDFLARE_D1_DATABASE_ID=your-database-id
          CLOUDFLARE_API_TOKEN=your-api-token

      (requires the python-dotenv package; falls back gracefully if it
      isn't installed, in which case set real environment variables
      instead.)

   b) Deployed via Streamlit Community Cloud (or `streamlit run` with a
      secrets.toml) — add the same three keys to .streamlit/secrets.toml,
      or the platform's Secrets UI once deployed:

          CLOUDFLARE_ACCOUNT_ID = "your-account-id"
          CLOUDFLARE_D1_DATABASE_ID = "your-database-id"
          CLOUDFLARE_API_TOKEN = "your-api-token"

_get_secret() checks Streamlit secrets first, then falls back to the
environment — so the same code works in both places without changes.

Free tier: 5 million row reads/day, 100,000 row writes/day, resetting
daily at 00:00 UTC — comfortably more than a single classroom competition
should need.
--------------------------------------------------------------------------
"""
